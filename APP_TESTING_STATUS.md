...
- DEF-008 resolved: the remaining long tail of individual account/company Select pickers across Journal, Tax, and Sales/Purchase order-form company fields (plus their table filters) are now company-scoped — see `APP_TESTING_ERRORS.md` for the full per-file breakdown.
...
- ~~`RefundResourceTest.php`'s fixtures need the same `accounting_post_journal` permission grant DEF-009 already added.~~ **Fixed** (ENV-008): 3/8 → 7/8 passing. Surfaced a distinct issue (ENV-009), now also **fixed**: it wasn't a `PaymentRegister` amount-computation bug -- the Pay action's default journal selection ignored currency entirely, so an arbitrarily-ordered, currency-mismatched real journal could become the default and trip the app's own strict exchange-rate guard. Also independently confirmed to affect `InvoiceResourceTest`'s "equivalent" Pay test, which this session's original ENV-008 finding had assumed was unaffected. `PaymentRegister::computeAvailableJournalIds()` now prefers a same-currency journal for the default while leaving every journal selectable. `RefundResourceTest.php`: 7/8 → 8/8; `InvoiceResourceTest.php`: 8/9 → 9/9. Full `accounts` suite confirmed regression-free via `git stash` A/B (5 failed/511 passed → 3 failed/513 passed; the 3 remaining failures are pre-existing and unrelated -- 2 mail/partner-email-autofill assertions already flagged under DEF-009, plus one unrelated period-lock test). See `APP_TESTING_ERRORS.md`'s ENV-009 write-up for the full trace. A separate, out-of-scope bug was found in passing (`PaymentRegister::getBatchAvailableJournals()`'s inbound/outbound `PaymentType` enum comparison is always false) and flagged for dedicated follow-up rather than fixed here.
