...
{
    /**
     * The shared seeded company used across this suite. Backfilled here (not
     * in the seeder itself) so that every existing test posting a taxed
     * invoice keeps doing so unchanged now that sales-tax registration is
     * enforced at post time — tests that specifically want an unregistered
     * company or a missing STRN create their own Company instead of calling
     * this helper (see TaxComplianceTest).
     */
    public static function company(): Company
    {
        $company = Company::query()->firstOrFail();

        if (! $company->is_sales_tax_registered || blank($company->strn)) {
            $company->forceFill([
                'is_sales_tax_registered' => true,
                'strn'                    => $company->strn ?? '1234567-8',
            ])->save();
        }
