...
| ENV-008 (new) | `RefundResourceTest.php`'s fixtures were never updated for DEF-009 (only `InvoiceResourceTest`/`BillResourceTest` were). 5 of its lifecycle-action tests (pay, cancel, reset-to-draft, set-as-checked, register-payment) now correctly fail with "action ... is visible on the component ... Failed asserting that false is true" because their `actingAs` fixtures don't grant `accounting_post_journal`. Found while verifying the ENV-007 fix; pre-existing (confirmed via the actual assertion message matching DEF-009's fix, not the DB rebuild) and unrelated to it. Needs the same fixture update `InvoiceResourceTest`/`BillResourceTest` already got. |
