...
| ENV-009 | ~~`registers a full payment and marks the refund paid through the action` (`RefundResourceTest.php`) clears the permission gate and `PayAction` runs with zero validation errors (`assertHasNoActionErrors()` passes), but `payment_state` stays `NOT_PAID` instead of becoming `PAID`.~~ **RESOLVED** (2026-09-25). Not a `PaymentRegister` amount-computation bug as originally suspected — `PayAction`'s default journal selection picked an arbitrarily-ordered, currency-mismatched real journal, silently tripping the app's own strict exchange-rate guard. Turned out to also affect `InvoiceResourceTest`'s "equivalent" Pay test, contradicting this row's original premise. See write-up below. |
...
### ENV-009 fix write-up (2026-09-25): `PaymentRegister` default journal ignores currency

**Actual (before fix).** `RefundResourceTest.php`'s "registers a full payment and marks the refund paid" test: `PayAction`'s form validated with zero errors (`assertHasNoActionErrors()` passed) but `payment_state` stayed `NOT_PAID`.

**Root cause.** Not the `PaymentRegister`/`getTotalAmountsToPay()` amount-computation bug this row originally suspected. Reproducing the full flow directly (`php artisan tinker` against `aureuserp_testing`, building a real `PaymentRegister` the same way `PayAction::schema()` does) showed the payment amount, batching and journal typing were all already correct. The actual failure: `PaymentRegister::computeAvailableJournalIds()` (`plugins/webkul/accounts/src/Models/PaymentRegister.php:260`) returns candidate journals in whatever order MySQL happens to return them in — with no regard for currency — and both `PayAction`'s form default (`$paymentRegister->available_journal_ids[0]`) and `AccountHelper::pay()`'s test helper blindly take index `[0]` as *the* default journal. `aureuserp_testing` was rebuilt in ENV-007 as a full copy of the real dev DB, which includes the company's two real, fully-configured production journals ("Bank Transactions", "Cash Transactions" — both PKR). Those real journals' low, stable IDs (5, 6) reliably sort ahead of whatever high-numbered journal the test's own `AccountHelper::bankJournal()` creates in that run, so the PKR journal — not the fixture's own same-currency (USD, from `AccountHelper::currency()`) bank journal — became the default. `PayAction::action()` (`PayAction.php:335-345`) then correctly runs a `strict: true` exchange-rate conversion check (a deliberate safeguard added earlier this session — see the "Stop payment registration from silently using a 1:1 exchange rate" fix in git history) between the refund's own currency and the payment's currency; since no USD→PKR rate exists in the test DB, it throws `RuntimeException: There's no exchange rate set up for converting USD to PKR...`. `PayAction::action()`'s own `catch (Throwable $e)` swallows this into a `Notification` and calls `$this->halt(shouldRollBackDatabaseTransaction: true)` — which rolls back the payment but is *not* a form validation error, so `assertHasNoActionErrors()` still passed while nothing was actually recorded.

This is a genuine application bug, not a test-fixture-only gap: confirmed by running `InvoiceResourceTest`'s "registers a full payment and marks the invoice paid" test in isolation (`php artisan test --filter=...`) before this fix — it *also* failed (with a derivative `ActionNotResolvableException` from the same mid-flight `halt()` corrupting the Livewire test harness's mounted-action state), directly contradicting this row's original premise that the invoice case worked. Any real multi-currency company where the default-picked journal doesn't happen to match the document's currency would hit this same silent failure in production, not just in tests.

**Fix.** `PaymentRegister::computeAvailableJournalIds()` now sorts the (still-complete, still-deduplicated) list of available journals so that a journal whose `currency_id` matches the register's own `currency_id` sorts first, using a stable sort so relative ordering is otherwise unchanged. Every previously-available journal remains available and selectable in the form — a user can still manually pick a different-currency journal, at which point the existing strict exchange-rate check still correctly applies. Nothing about batching, reconciliation, posting, or the exchange-rate guard itself was touched.

**Verified:**
- Reproduced the exact failure via `php artisan tinker` (`DB_DATABASE=aureuserp_testing`) both before and after the fix, confirming `available_journal_ids[0]` changes from the real PKR journal (id 5) to the fixture's own USD journal.
- Confirmed the same latent bug independently affected `InvoiceResourceTest`'s "registers a full payment and marks the invoice paid" test (`ActionNotResolvableException`, a side effect of the mid-flight `halt()`) before this fix, and that this fix resolves it too (9/9 passing).
- `vendor/bin/pint --test` on the changed file: passed, no formatting changes needed.

**Tests:**
- `RefundResourceTest.php`: 7/8 → **8/8 passing**.
- `InvoiceResourceTest.php`: 8/9 → **9/9 passing** (not previously tracked as broken, but empirically was).
- Full `plugins/webkul/accounts` suite, `git stash`/`stash apply` A/B: baseline (without this fix) 5 failed/511 passed; with this fix, 3 failed/513 passed. The 2 newly-fixed tests are `InvoiceResourceTest`'s and `RefundResourceTest`'s Pay-action tests above. The remaining 3 failures are confirmed pre-existing and unrelated (identical in both runs): `BillResourceTest`'s "sends paid bill email" and "updates partner email" tests (both pass explicit `journal_id`/`currency_id` in their `callAction()` data, never touching the default-selection path this fix changes; already flagged in the DEF-009 write-up as a known mail/partner-email-autofill gap), and `CurrencyTest`'s "creates an exchange-d..." test (blocked by an unrelated period-lock guard, nothing to do with journal selection). A `FiscalPositionTest` API test flaked once in an earlier full-suite run (Faker-seed/order dependent, unrelated to accounting code) and passed cleanly in both the controlled before/after comparison runs.

**Not fixed / separate, out-of-scope finding:** while tracing this, `PaymentRegister::getBatchAvailableJournals()` (`PaymentRegister.php:463-477`) was found to compare `$paymentType == 'inbound'` where `$paymentType` is a `PaymentType` enum instance, not a string — a `BackedEnum` is never loosely equal to its own raw scalar `->value` in PHP, so this comparison is always `false` and the method always queries `outboundPaymentMethodLines`, never `inboundPaymentMethodLines`, regardless of the payment's real direction. This did not contribute to ENV-009 (the two real seeded journals have both inbound and outbound payment method lines configured), so fixing it wasn't necessary to resolve this row, and changing which payment-method-line relation gates journal availability system-wide is a broader change than this fix's scope. Flagged separately for a dedicated fix.

**Files:** `plugins/webkul/accounts/src/Models/PaymentRegister.php` (`computeAvailableJournalIds()`).
